...
const validate = (piece: ChessPiece, gameBoard: GameBoard, x: number, y: number, isDiagonal = false) => gameBoard.validateCoord({ x, y })
...
const validateDiagonal = ({ gameBoard, piece, direction, validMoves, oneStep }: ValidationProps) => {
  const forwardLeft: Position = { x: piece.position.x - 1, y: piece.position.y + direction };
  const forwardRight: Position = { x: piece.position.x + 1, y: piece.position.y + direction };
  const backwardsLeft: Position = { x: piece.position.x - 1, y: piece.position.y - direction };
  const backwardsRight: Position = { x: piece.position.x + 1, y: piece.position.y - direction };
